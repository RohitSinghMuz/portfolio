...
const MyPortfolio=()=>{
  const gitHubFun=()=>{
       window.open('https://github.com/RohitSinghMuz', '_blank');
  }
  const githubLinkFun =(linkgithubProject)=>{
         window.open(linkgithubProject, '_blank');
         console.log("linkgithubProject",linkgithubProject)
  }
  return (
    <section className="portfolio--section" id="MyPortfolio">
      <Box className="portfolio--container-box">
        <Box className="portfolio--container">
          <Typography className="sub--title">Recent Projects</Typography>
          <h2 className="section--heading">My Portfolio</h2>
        </Box>
        <Box>
          <button className="btn btn-github" onClick={gitHubFun}>
            <svg
      ...
      <Box className="portfolio--section--container">
        {data?.portfolio?.map((item, index) => (
          <Box key={index} className="portfolio--section--card">
            ...
            <Box className="portfolio--section--card--content">
              ...
              <Typography className="text-sm portfolio--link" onClick={()=>githubLinkFun(item.link)}>
              View in Github
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
